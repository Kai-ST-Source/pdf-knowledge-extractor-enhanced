#!/usr/bin/env python3
"""
Make the icon boundary much smaller to exactly match other macOS apps
"""
...
def create_much_smaller_boundary_icon(source_path, output_dir):
    """
    Create icon with much smaller overall boundary
    """
    ...
    for size, filename in icon_configs:
        ...
        # Make the visible area much smaller - try 75% instead of 85%
        visible_area_ratio = 0.75  # Much smaller visible area
        ...
        # Use most of the visible area for content (95% instead of 90%)
        content_ratio_in_visible = 0.95  # Maximum content within the small boundary
        ...
        corner_radius = max(1, int(visible_size * 0.15))  # Slightly more rounded
        ...
        print(f"Created {filename}: boundary {visible_size}x{visible_size} (75% of {size}), content {content_size}x{content_size} (95% of boundary)")
    
    # Create .icns
    icns_path = os.path.join(output_dir, "PDFKE_smaller.icns")
    try:
...
if __name__ == "__main__":
    ...
    if os.path.exists(source_file):
        print("Creating icon with much smaller boundary...")
        print("Strategy: 75% visible area, 95% content within that small area")
        print()
        
        icns_file = create_much_smaller_boundary_icon(source_file, ".")
        if icns_file:
            print(f"\n✓ Created smaller boundary icon: {icns_file}")
            print("The overall icon boundary is now much smaller")
        else:
